...
keywords = [
    ...
    "Modelo de transporte", "Encuesta origen destino", "Toma informacion de campo",
    "Caracterizacion de vías", "Estudio de tránsito", "Diseño Señalización",
    ...
    "políticas públicas", "Estudios de comportamiento", "Planificación de desarrollos",
    "Planificación de infraestructura", "Analítica espacial", "Recaudación",
    "gestión de tarifas"
]
...
class SecopDataFetcher:
    ...
    def fetch_all_data(self, page_size: int = 1000) -> List[Dict[str, Any]]:
        ...
        params = [(offset, page_size) for offset in range(0, total_records, page_size)]
        all_data = []
...
def process_data(data: List[Dict[str, Any]], is_secop_ii: bool = True) -> pd.DataFrame:
    ...
    if is_secop_ii:
        ...
        numeric_columns.extend([col for col in df.columns 
    ...
    if useFilterKeywords:
        ...
        df = df[df[desc_field].str.contains(pattern, case=False, na=False, regex=True)]
...
def fetch_and_process_all_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Main function to fetch and process data from both SECOP I and II."""
    secop_ii_fetcher = SecopDataFetcher("https://www.datos.gov.co/resource/p6dx-8zbt.json", True)
    secop_i_fetcher = SecopDataFetcher("https://www.datos.gov.co/resource/jbjy-vk9h.json", False)
    
    # Fetch and process SECOP II data
    print("\nProcessing SECOP II data...")
    secop_ii_data = secop_ii_fetcher.fetch_all_data()
    df_secop_ii = process_data(secop_ii_data, True)
    if not df_secop_ii.empty:
        save_to_csv(df_secop_ii, 'secop_ii_transport_presentation_phase')
        generate_category_summary(df_secop_ii)
        
    # Fetch and process SECOP I data
    print("\nProcessing SECOP I data...")
    secop_i_data = secop_i_fetcher.fetch_all_data()
    df_secop_i = process_data(secop_i_data, False)
    if not df_secop_i.empty:
        save_to_csv(df_secop_i, 'active_transport_contracts')
        generate_transport_summary(df_secop_i)
    
    return df_secop_i, df_secop_ii
...
def generate_category_summary(df: pd.DataFrame) -> None:
    ...
    print(f"Unique category codes: {df['codigo_principal_de_categoria'].nunique()}")
...
if __name__ == "__main__":
    df_secop_i, df_secop_ii = fetch_and_process_all_data()
